...
if __name__ == '__main__':
    parser = OptionParser(
        usage='usage: %prog [-c,-p] [-d,--dir] DIRECTORY [-f,--file] FILE [-n,--name] NAME\n'
    ...
    parser.add_option(
        ...
        help='Show a nice progressbar (no effect with when combined with the gui)'
    )
    parser.add_option(
    ...
    if len(args) > 1 and not options.input_dir and not options.file and not options.name and all(
            os.path.isdir(elem) for elem in args):
        for elem in args:
            ...
            if not tail:
                ...
                input_dir = args
            creator = EPubMaker(None, input_dir, input_dir + '.epub', tail, progress=CmdProgress(options.progress))
            creator.run()
    else:
        if not options.input_dir and args:
            options.input_dir = args.pop(0)
        ...
        if options.cmd:
            if args or not options.input_dir or not options.file or not options.name:
                ...
                creator = EPubMaker(
                    None, options.input_dir, options.file, options.name, progress=CmdProgress(options.progress)
                )
                creator.run()
        else:
            import _Gui

            _Gui.start_gui(input_dir=options.input_dir, save_file=options.file, str_name=options.name)
